...
class RosiNodeClass():
	...
	# how to obtain these values? see Mandow et axes_lin. COMPLETE THIS REFERENCE
	var_lambda = 0.965
	wheel_radius = 0.1324
	ycir = 0.531
	...
	def __init__(self):
		...
		self.sub_cmdvel = rospy.Subscriber('/cmd_vel', Twist, self.callback_CmdVel)
	...
	def callback_CmdVel(self, msg):
		vlin_x = msg.linear.x
		vlin_y = msg.linear.y
		vang_z = msg.angular.z

		vel_linear_x = self.max_translational_speed * vlin_x
		vel_angular_z = self.max_rotational_speed * vang_z
		...
		self.omega_right = x[0][0]#np.deg2rad(x[0][0])
		self.omega_left = x[1][0]#np.deg2rad(x[1][0])
